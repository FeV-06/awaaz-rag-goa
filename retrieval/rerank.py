...
class ONNXCrossEncoderRanker:
    ...
    def _ensure_onnx_model(self):
        """Auto-export CrossEncoder to ONNX. INT8 quantization is opt-in via
        CE_INT8=true — dynamic quantization of this classifier degrades its
        logits (verified: correct passage scored 0.99 fp32 vs 0.03 int8),
        which breaks the whole relevance gate."""
        if self.onnx_int8_path.exists() or self.onnx_fp32_path.exists():
            if not self.onnx_int8_path.exists() and self.onnx_fp32_path.exists() and os.getenv("CE_INT8", "false").lower() == "true":
                try:
