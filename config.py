...
# ==========================================
# Active languages for the deployed runtime. Keep as single source of truth.
LANGUAGES = ["en", "hi", "mr"]
...
FAISS_TOP_K = 15
RERANK_TOP_K = 10
# BM25-hybrid fusion is the fallback re-ranker (used only when the cross-encoder
# is disabled). Kept dense-dominant: e5 scores are tightly clustered, so heavy
# lexical weight lets rare-token lookalikes crowd out true matches.
HYBRID_BM25_WEIGHT = 0.15

# Cross-Encoder Re-Ranking Configuration
# ON by default: BM25+dense fusion alone can rank transliteration lookalikes
# (e.g. "FAISS" vs. a drug named "फोसामॅक्स") above the true match; the
# multilingual cross-encoder fixes this. Measured ~53-103ms CPU ONNX.
...
CROSS_ENCODER_TOP_K = int(os.getenv("CROSS_ENCODER_TOP_K", "8"))
...
# Set AWAZ_ENABLE_LLM=true to route generation through the configured LLM provider
# (default: local Ollama GPU endpoint — no external API needed, privacy-first).
ALLOW_NETWORK_CALLS_IN_PIPELINE = os.getenv("AWAZ_ENABLE_LLM", "false").lower() == "true"
...
# Tier-1 Primary: Local Ollama GPU (OpenAI-compatible) — privacy-first default.
# Swap LLM_BASE_URL / LLM_MODEL / LLM_API_KEY for any OpenAI-compatible provider.
LLM_API_KEY = os.getenv("LLM_API_KEY", "local-ollama")
