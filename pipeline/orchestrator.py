...
class RAGPipelineOrchestrator:
    ...
    async def execute(self, request: QueryRequest) -> QueryResponse:
        ...
        # Fast regex blocklist + Meta Prompt-Guard-86M local ONNX discriminator
        ...
        def _rerank_candidates(q_text, c_dicts):
            if getattr(config, "ENABLE_CROSS_ENCODER", False):
                # Cross-encoder adjudicates the RRF-merged candidate pool
                # directly. (A lexical pre-rescore here is counterproductive:
                # e5 dense scores are tightly clustered, so any BM25 weight
                # lets rare-token lexical lookalikes crowd out the true match
        ...
        if config.ENABLE_LOCAL_SLM:
            ...
            # Multi-source compilation & grounded synthesis via configured LLM provider (default: local Ollama)
            context_blocks = []
            ...
            if "don't have enough grounded information" in candidate_answer.lower():
                ...
                gen_details = f"Cross-lingual multi-source synthesis into '{target_lang}' via {config.LLM_MODEL}"
            else:
